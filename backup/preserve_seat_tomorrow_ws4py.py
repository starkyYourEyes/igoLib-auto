...
from backup.keep_session import *
...
def initialization():
    ...
    for usr in usr_list:
        ...
        """把座位name改为座位key"""
        libLayout_operation['variables']['libId'] = usr['lib_id']
        all_seats = requests.post(
            url=url, headers=pre_header, json=libLayout_operation, verify=False
        ).json()
        ...
        usr_info = requests.post(url=url, headers=pre_header, json=index_operation, verify=False).json()
    ...
    try:
        for _ in range(3):
            for seat in usr['seats']:
                save_operation['variables']['key'] = seat + '.'
                text_save = session.post(  # 抢座的post请求
                    url=url,
                    headers=pre_header,
                    json=save_operation,  # save
                    verify=False
                ).text
                save_end = time.time()
...
def preserve_tomorrow(session, usr: dict):
    ...
    save_operation['variables']['libid'] = usr['lib_id']
