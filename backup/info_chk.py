...
if __name__ == '__main__':
    ...
    usr_list[0]['session'] = requests.session()
    session = usr_list[0]['session']
    res = session.post(url=url, json=params_index, headers=headers, verify=False)
    session.headers = res.request.headers
    print(session.cookies)
    new_cookie = requests.utils.dict_from_cookiejar(session.cookies)
    print(new_cookie)
    #
    # print(session.cookies, type(session.cookies))
    # print(res.request.headers['Cookie'],  type(res.request.headers['Cookie']))
    # res2 = session.post(url=url, json=params_index, verify=False)
