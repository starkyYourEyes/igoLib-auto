...
def initialization():
    ...
    for user in user_list:
        """把座位name改为座位key"""
        data_lib_chosen['variables']['libId'] = user.lib_id
        ...
        user_info = user.session.post(
            url=url, headers=pre_header, json=params_index, verify=False
        ).json()
...
def seat_save(open_time, user):
    ...
    if time.time() - open_time >= 5:  # for henan # ?????
        ...
        print('time in data_lib_chosen:', time.time() - queue_end)
    try:
...
async def queue_pass_websockets(open_time, user):
    ...
    queue_header_for_websockets['Cookie'] \
        = User.cookie_dict_to_str_with_equal(requests.utils.dict_from_cookiejar(user.cookie))
            ...
            if time.time() >= start_queue_time:
                while True:  # 内循环控制websocket通信，直到收到排队成功的消息
                    try:
                        ...
                        print(e)
                        if time.time() - open_time >= 80 or save_round >= 5:  # 保证程序退出
                            break
                        time.sleep(0.8)
                break  # 跳出外层循环
    if not success_pre_reserve:
        my_email.goLib_email_info('fail', receiver=user.email)


def preserve_tomorrow(user: User):
    params_confirm_seat['variables']['libid'] = user.lib_id
    # 更新为当日抢座时间,open_time -> 时间戳
    open_time = time.mktime(
        time.strptime(
            time_update(user.pre_reserve_time), "%Y-%m-%d %H:%M:%S"))
    print('🙏明日预约初始化完成！')
...
if __name__ == '__main__':
    ...
    print('currently', len(user_list), 'users active')
