...
from library_freedom import typical_one
...
def get_token_and_exp_date_str(session, usr):
    # 通过这个请求对应的响应体来获取取消预约的令牌哦
    # 以及最晚签到时间的时间戳
    headers['Cookie'] = usr['cookie']
    json_data = session.post(url=url, headers=headers, json=index_param, verify=False).json()
    # print('=============================headers.item()=======================================')
...
def withdraw_seat(session, usr):
    # print('开始退座')
    # print('token = ', token)
    # print(res)
    try:
        exp_date, token = get_token_and_exp_date_str(session, usr)
        reserve_cancel["variables"]["sToken"] = token
        headers['Cookie'] = usr['cookie']
        res = session.post(url=url, headers=headers, json=reserve_cancel, verify=False).json()
        if res["data"]["userAuth"]["reserve"]["reserveCancle"]["timerange"] > 0:
            print('退座成功！', time.ctime())
        else:
...
#                 reserve_cancel['variables']['sToken'] = token
#                 headers['Cookie'] = usr['cookie']
#                 quit_seat = session.post(url=url, headers=headers, json=reserve_cancel, verify=False).json()
#                 try:
